...
\paragraph{Observations diverses} % (fold)
...
\begin{enumerate}
	\item On observe des profils très similaires entre \esp{wTriaur} et \esp{wAur}, ce qui peut s’expliquer par la très forte apparenté de ses hôtes.
	\item \esp{wSub} semble partager beaucoup fragments avec \esp{wSuz}, témoin aussi d’une forte apparenté de ses hôtes. Il serait intéressant de séquencer afin de confirmer l’égalité de ces fragments, et d’identifier l’insertion supplémentaire.
	\item Une souche de \esp{wMel} issue d’une lignée de Madagascar présente un fragment supplémentaire (wMel2 sur la figure \ref{fig:profils}). Il faudrait confirmer cette observation en échantillonnant plus d’individus de cette lignée.
\end{enumerate}
% paragraph observations_diverses (end)
